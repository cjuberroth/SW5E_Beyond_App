...
const useAPIData = () => {
    const [api_Species, set_api_Species] = useState(null)
	const [api_Class, set_api_Class] = useState(null)
	const [api_Feat, set_api_Feat] = useState(null)
	const [api_Power, set_api_Power] = useState(null)
	const [api_Archetype, set_api_Archetype] = useState(null)
	const [api_ArmorProperty, set_api_ArmorProperty] = useState(null)
	const [api_Background, set_api_Background] = useState(null)
	const [api_Conditions, set_api_Conditions] = useState(null)
	const [api_EnhancedItem, set_api_EnhancedItem] = useState(null)
	const [api_Equipment, set_api_Equipment] = useState(null)
	const [api_Feature, set_api_Feature] = useState(null)
	const [api_FightingMastery, set_api_FightingMastery] = useState(null)
	const [api_FightingStyle, set_api_FightingStyle] = useState(null)
	const [api_LightsaberForm, set_api_LightsaberForm] = useState(null)
	const [api_Maneuvers, set_api_Maneuvers] = useState(null)
	const [api_WeaponFocus, set_api_WeaponFocus] = useState(null)
	const [api_WeaponProperty, set_api_WeaponProperty] = useState(null)
	const [api_WeaponSupremacy, set_api_WeaponSupremacy] = useState(null)
	const [api_SkillsLU, set_api_SkillsLU] = useState(null)
